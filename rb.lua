...
local function GetNearestLane(position) -- table.sort cant be used here
    ...
    for laneIndex, lanePosition in next, Autoplayer.lanePositions[Autoplayer.currentLanePositionsIndex] do
        ...
        if distance  2 then
            ...
            elseif instance.Transparency  0.2 and math.floor(instance.CFrame.Y * 10) == Autoplayer.sliderY then
                ...
                if noteLane then
                    ...
                    if not Autoplayer.heldLanes[noteLane] and distance  2 and math.abs((instance.CFrame + instance.CFrame.LookVector * instance.Height / 2).X - lanePosition.X) or (instance.CFrame + instance.CFrame.LookVector * instance.Height / 2).X - lanePosition.X) &lt;= randomDistance
                        VirtualInputManager:SendKeyEvent(false, keys[noteLane], false, game)
